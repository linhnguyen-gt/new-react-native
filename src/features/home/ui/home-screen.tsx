...
import { useGetResponseQuery } from '@/features/response/api/response-api';
...
// Count actions only — the response spinner belongs to RTK Query now.
const LOADING_TYPES = [CountActions.increment.type, CountActions.decrement.type];
...
const MainPage = () => {
    ...
    // RTK Query fetches on mount and owns its loading and error state, so the useEffect that
    // dispatched a saga on mount — and the three reducers behind it — are gone.
    const { data: response, isFetching, error, refetch } = useGetResponseQuery();
                    ...
                    {error ? (
                        <VStack space="xs" alignItems="center">
                            <Text size="sm" color="red" textAlign="center" testID="response-error">
                                {'message' in error ? error.message : 'The request failed'}
                            </Text>
